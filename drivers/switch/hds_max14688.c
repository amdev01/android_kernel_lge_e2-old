...
#define JACK_MATCH(_name, _mic, _left, _has_btn, _mode0, _mode1, \
_switch_name, _switch_state, _evt_type, _evt_code1, _evt_code2) \
{\
	.name           = _name,\
	.mic            = _mic,\
	.left           = _left,\
...
#define log_err(format, ...) \
	pr_err(log_fmt(format), ##__VA_ARGS__)
#undef  log_warn
...
{\
	if (likely(max14688_log_level >= 0)) {\
		pr_info(log_fmt(format), ##__VA_ARGS__);\
	} \
}
#undef  log_dbg
...
{\
	if (likely(max14688_log_level >= 1)) {\
		pr_debug(log_fmt(format), ##__VA_ARGS__);	\
	} \
}
#undef  log_vdbg
...
static inline int max14688_i2c_seq_read(struct max14688 *me,
...
static inline int max14688_i2c_seq_write(struct max14688 *me,
...
static void max14688_irq_jack_inserted(struct max14688 *me)
{
	...
	if (unlikely(max14688_get_status(me, STATUS_INT)
				&& max14688_get_status(me, STATUS_MICIN))) {
		...
		struct device_attribute *devattr, const char *buf, size_t count)
{
	...
	max14688_log_level = (int)kstrtol(buf, 10, &log_level);
		...
		struct device_attribute *devattr, const char *buf, size_t count)
{
	...
	delay = (unsigned int)kstrtol(buf, 10, &temp_delay);
		...
		struct device_attribute *devattr, char *buf)
{
	...
	rc += (int)snprintf(buf+rc, PAGE_SIZE,
			...
			BITS_GET(reg_val, PINCONTROL1_MODE1) == 0x0b00 ? "low" :
			BITS_GET(reg_val, PINCONTROL1_MODE1) == 0x0b11 ? "high" :
			"in high Z");
	rc += (int)snprintf(buf+rc, PAGE_SIZE,
			"  MODE0 (Microphone Bias) is %s\n",
			BITS_GET(reg_val, PINCONTROL1_MODE0) == 0x0b00 ? "low" :
			BITS_GET(reg_val, PINCONTROL1_MODE0) == 0x0b11 ? "high" :
			"in high Z");
...
{\
	...
	long temp_val;\
	__lock(me);\
	val = (int)kstrtol(buf, 16, &temp_val);\
...
static bool max14688_detect_jack(struct device *dev)
{
	...
	return (max14688_get_status(me, STATUS_DET) == 0) ? true : false;
}
...
static int max14688_read_mic_impedence(struct device *dev)
{
	...
	rc = qpnp_vadc_read(input_vadc, vadc_channel, &result);
	log_info("%s: vadc channel is %d\n", __func__, vadc_channel);
	if (rc < 0) {
		if (rc == -ETIMEDOUT)
...
static int max14688_read_left_impedence(struct device *dev)
{
	...
	/* Greater than 2.69k(ohm) resistor is connected,
	   read the EOC bit in the ADCSTATUS address */
	max14688_read(me, ADCSTATUS, &adcstatus);
...
{
	struct max14688 *me = container_of(work, struct max14688,
...
static void max14688_parse_dt(struct device *dev, struct max14688_platform_data *pdata)
{
	...
	rc = of_property_read_u32(np, "qcom,vadc-channel", &vadc_channel);
	if (rc) {
		vadc_channel = P_MUX7_1_1; /* default channel */
		log_err("%s: can't qcom,vadc-channel => set default setting %d\n",
			   __func__, vadc_channel);
	}
}
