...
int mts_tty_process(char *buf, int left)
{
    ...
    switch (mts_tty->run) {
        ...
        case MTS_STOP_READY_COMPLETE:
            printk("mts_tty: no push\n");
            return -1;
        case MTS_STOP_READY:
            printk("mts_tty ready_to_stop\n");
...
static int mts_tty_write(struct tty_struct *tty, const unsigned char *buf, int count)
{
    ...
    //printk("%s %s %d %d\n", __func__, (const char *)buf, (int)count, mts_tty->count);
#if 0
    diag_process_hdlc((void *)buf + 4, count - 4);
#endif


#if 1
    driver->usb_buf_out = (unsigned char *)buf + 4;
    driver->read_len_legacy = count - 4;
    queue_work(driver->diag_wq, &(driver->diag_proc_hdlc_work));
#endif

    printk("mts_tty - written bytes: %d\n", count - 4);
